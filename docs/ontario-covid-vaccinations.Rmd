...
## Clean data  
# Slice Ontario population estimates to obtain a value for population over 10.
ONpop2020 <-
...
ONpop2020kids <-
  ONpop2020 %>% slice(1:2) # remove age categories 0-4, 5-9 from all population.
ONpop2020kids <-
  sum(ONpop2020kids["VALUE"]) # Sum the age if categories 0-4, 5-9
ONpop2020adults <-
  ONpop2020all - ONpop2020kids # Subtract sum total of values 0-4, 5-9from total population to obtain adult population estimate.
```
...
# The province administered the first vaccine on 2020-12-14 but didn't record data until 2020-12-24. Average the reported figure for the first day in the data set over the previous ten days.
...
christmas2020data <- data.frame(
  ...
  "previous_day_doses_administered" = c(firstdoses, 0, 0, 0, 0),
...
vaxx$none <- ((ONpop2020adults - vaxx$fullorpart))

# Percentage shares
vaxx$percentagepart <-
  ((vaxx$part / ONpop2020adults) * 100) # Part vaccinated
vaxx$percentagefully <-
  ((vaxx$total_individuals_fully_vaccinated / ONpop2020adults) * 100) # Fully vaccinated
vaxx$percentagefullorpart <- ((vaxx$fullorpart / ONpop2020adults) * 100) # Fully or partly vaccinated
vaxx$percentagenone <- ((vaxx$none / ONpop2020adults) * 100) # No vaccination

# Round percentages to two decimals
vaxx$percentagefully <- round(vaxx$percentagefully, digits = 2)
vaxx$percentagepart <- round(vaxx$percentagepart, digits = 2)
vaxx$percentagefullorpart <- round(vaxx$percentagefullorpart, digits = 2)
vaxx$percentagenone <- round(vaxx$percentagenone, digits = 2)
```
...
ggplot(bar, aes(x = "", y = Percentage, fill = Status)) +
  ...
  labs(
    ...
    title = "Ontario COVID-19 vaccination status of adult population (aged 10+)",
    subtitle = Sys.Date()
  ) +
  theme_classic() +
  theme(
...
ggplot(vizfull, aes(report_date, percentage)) +
  ...
  labs(
    ...
    subtitle = "Percentage of adult population (aged 10+)"
  ) +
