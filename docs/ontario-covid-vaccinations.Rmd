...
fsa1 <- read_xlsx("ICES-COVID19-Vaccination-Data-by-FSA.xlsx", sheet = "At least 1 Dose Vaccine by FSA", skip = 23, col_names = TRUE)
fsa2 <- read_excel("ICES-COVID19-Vaccination-Data-by-FSA.xlsx", sheet = "Both Dose Vaccination by FSA", skip = 23, col_names = TRUE)
```
...
# We only want to keep the FAS region and percentage vaccinated for all ages columns.
...
fsavaxx <- merge(fsa1, fsa2, by.x = "FSA")
```
...
region_viz1 <- region_mean %>% select(Region, onedose)
region_viz2 <- region_mean %>% select(Region, bothdose)
...
region_viz <- rbind(region_viz1, region_viz2)
