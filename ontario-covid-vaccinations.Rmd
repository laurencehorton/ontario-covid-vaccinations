...
non_hospital_prev <-
  ...
  ) %>% select(report_date, covid19_cases_unvac_prev:covid19_cases_full_vac_prev) #%>% mutate(across(where( ~ is.numeric(.)), ~ round(., 0)))
```
...
hospital_prev <-
  ...
  ) %>% select(report_date, hospital_unvac_prev:hospital_full_vac_prev) #%>% mutate(across(where( ~ is.numeric(.)), ~ round(., 0)))
```
...
icu_prev <-
  ...
  ) %>% select(report_date, icu_unvac_prev:icu_full_vac_prev) #%>% mutate(across(where(~ is.numeric(.)), ~ round(., 0)))
```
...
ggplot(data = breakthrough_prev_viz, aes(x = report_date, group = 1)) +
